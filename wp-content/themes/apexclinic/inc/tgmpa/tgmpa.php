...
add_action( 'tgmpa_register', 'apexclinic_register_required_plugins' );
function apexclinic_register_required_plugins() {
	$plugins = array(
	...
	$response = wp_remote_get( 'https://api.radiantthemes.com/plugins/apexclinic/version-one.php', $args );
	...
	if( isset( $data['error'] ) ) {
		echo new WP_Error( 'invalid_response', $data['error'] );
	}
